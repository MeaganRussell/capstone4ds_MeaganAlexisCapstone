...
format: 
  html:
    ...
    include-in-header:
      text: |
        <style>
        /* Forces Figures and Tables to match exactly */
        figcaption,tab_header, title, name, caption, .caption-prefix, .caption-number, .quarto-xref {
...
variable_table %>%
  gt() %>%
  tab_header(
    title = "Table 1: Variables Used in Random Forest Model"
  ) %>%
  opt_align_table_header(align = "center")


```
Floods are categorized as Flash Flood or Flood. In Northwest Florida there have been 1023 flood events, Figure 2. 
...
ggplot(flood_counts, aes(x = "", y = percent, fill = EVENT_TYPE)) +
  ...
  labs(
    title = "Figure 2: Flash Flood vs Flood Events (2000–Present)",
    fill = "Event Type"
  ) +
  scale_fill_manual(values = c("Flash Flood" = "blue2", "Flood" = "chartreuse3")) +
  theme_void()
```
To further visual the number of floods in Northwest Florida, we broke down the type of flood per county in Figure 2. 
...
stormall %>%
  ...
  labs(
    title = "Figure 3: Flood and Flash Flood Events by County",
    x = "County",
    y = "Number of Events",
    fill = "Event Type"
  ) +
...
In Figure 3 b the number of flooding events from January 1, 2020 to December 31, 2025, for the 16 counties in Northwest Florida are shown. On the center y-axis damage in millions is shown and on the center y-axis the number of flood events is shown. The orange trend line tells us that the number of flood events is increasing and the red trend line shows us that the total property damage is increasing as well.
...
ggplot(yearly_data, aes(x = year)) +
  ...
  labs(
    title = "Figure 4: Flood Events and Property Damage Over Time",
    x = "Year"
  ) +
...
```
Below is table 2 with the variables from the 2024 American Community Survey - 5 Year estimates. 
```{r, fig.align="center"}
library(readxl)
library(dplyr)
library(gt)
...
census_data %>%
  ...
  gt() %>%
  tab_header(
    title = "Table 2: Population, Housing Units, and Median Home Value by County",
...
ggplot(monthly_avg, aes(x = month_name, y = avg_monthly_rain_in)) +
  ...
  labs(
    title = "Figure 5: Average Monthly Rainfall by County",
    x = "Month",
...
```
Average flood events per month over the last six years, Figure 5.
...
ggplot(monthly_floods, aes(x = month, y = avg_events)) +
  geom_col(fill = "blue3") +
  labs(
    title = "Figure 6: Average Number of Flood Events per Month",
    x = "Month",
...
Most intense rainfall events, figure 6. 
```{r, fig.align="center"}
library(dplyr)
library(lubridate)
library(ggplot2)
...
ggplot(rain_intensity, aes(x = month, y = max_rain)) +
  geom_col(fill = "blue3") +
  labs(
    title = "Figure 7: Maximum Daily Rainfall by Month",
    x = "Month",
