...
## Week 3 - Individual Research:

**Paper One: A review of flash-floods management: From hydrological modeling to crisis management**

This paper was written due to gaps between scientific flood forecasting and effective crisis management and decision-making. Current flood forecasting is complex and requires specialized training to understand. Climate models forecast an increase in flash floods due to climate change and urbanization. Not only are flash floods dangerous they are are also one of least predictable natural hazards. The authors state that flash floods have short response times, high spatial variability, and severely impact people and infrastructure. They emphasize that even with hydrological modeling and forecasting, many flash floods still result in lives lost and high damage.

The authors conducted a literature review and evaluated studies based on several factors: how forecasting was integrated with emergency decision-making, whether operational constraints (time, data, cost) were addressed, and whether the study provided actionable outputs. As for limitations, the authors state that flash-flood models are robust but operationally weak, and that most tools are focused on hazard prediction rather than impact or severity. The authors state that machine learning shows promise when it comes to flash flood prediction and impact estimation. Overall, there is a gap between the science and how it needs to be used by emergency managers.

@sadkou2024flashflood

**Paper Two: A study on geological structure prediction based on random forest method**

The Paleo-Asian Ocean closure timeline is highly debated among geologists. Some geologists argue that the ocean closed in the early Carboniferous, while others argue that the ocean did not close until the end of the Carboniferous Period. This study utilized Random Forest to predict the type of tectonic setting, arc versus rift, in order to better estimate when the ocean closed. The authors used the GEOROC database, collecting data for 28,000 basalt samples from arc and rift settings. They narrowed down the data to focus on 27 major and trace elements, including SiO₂, TiO₂, FEOT, Nb, Ta, etc. Once the data were narrowed, they were left with 6,554 training samples. The authors then collected 31 basic Carboniferous rock samples from the Xingmeng orogen to predict the tectonic setting. They focused primarily on the Random Forest model but also compared the results with a deep neural network (DNN) model.

The authors performed validation using ROC/AUC analysis, confusion matrices, and F1 scores. They found that Random Forest outperformed the DNN model, with an AUC score of 0.978 compared to 0.838. This study is important because Random Forest does not simply provide a yes/no classification, but instead produces a probability of the tectonic setting, arc vs rift. The model predicted an arc setting, which suggests that the ocean closed later rather than earlier but wasn't all at once. This is significant because it implies that the ocean remained open longer than currently believed and that the region was still in an active tectonic phase. However, there are major limitations to the study. The results are dependent on the quality of the global geochemical database, and chemical signatures can be altered by later geological processes.

@chen2022randomforest
